...
// Audit 2026-09-01 A8: opt-in JSON log output for log shippers / observability
// stacks. LOG_FORMAT=plain (default) keeps the existing human-readable format.
const LOG_FORMAT = (process.env.LOG_FORMAT || "plain").toLowerCase();
const LOG_LEVEL = (process.env.LOG_LEVEL || "info").toLowerCase();
const LEVELS = { trace: 10, debug: 20, info: 30, warn: 40, error: 50, ok: 30, cli: 30, exec: 30, http: 30, store: 30 };
function levelEnabled(l) { return (LEVELS[LOG_LEVEL] ?? 30) <= (LEVELS[l] ?? 30); }
...
function writeToFile(level, rawMessage) {
  if (!levelEnabled(level)) {
    // Still keep the in-memory buffer useful: only respect LEVEL for stream/file.
    return;
  }
  const line = LOG_FORMAT === "json"
    ? JSON.stringify({ ts: new Date().toISOString(), level: level.toUpperCase(), pid: process.pid, msg: stripAnsi(rawMessage) }) + "\n"
    : `[${new Date().toISOString()}] [${level.toUpperCase()}] [pid:${process.pid}] ${stripAnsi(rawMessage)}\n`;
