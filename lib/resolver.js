...
// CLI locations rarely change at runtime (installed by the user, not this app).
// Cache with a TTL so the hot /api/health + /api/installed endpoints stop
// spawning `where.exe`/`which` on every request (measured ~1.4s per call without).
// TTL 5 min; negative misses also cached so a transient PATH gap is tolerated.
const RESOLVE_TTL_MS = 5 * 60 * 1000;
const _resolveCache = new Map(); // cmdName -> { ts, value }

/**
 * Clears the resolveCommand cache (used by tests).
 */
...
export async function resolveCommand(cmdName) {
  ...
  const now = Date.now();
  const hit = _resolveCache.get(cmdName);
  if (hit && now - hit.ts < RESOLVE_TTL_MS) return hit.value;

  const value = await resolveCommandUncached(cmdName);
  _resolveCache.set(cmdName, { ts: now, value });
  return value;
}

/**
 * Resolves a command to its absolute executable path (uncached core).
 * @param {string} cmdName Name of binary (e.g. 'cline', 'gh', 'node', 'git')
 * @returns {Promise<string|null>}
 */
export async function resolveCommandUncached(cmdName) {
  if (!cmdName) return null;
