...
export function reconcile(state, probe) {
  ...
  for (const [key, item] of Object.entries(nextItems)) {
    ...
    if (type === "plugin") {
      ...
      stillThere =
        probe.found.plugins?.has(id) ||
        probe.found.plugins?.has(cleanPluginId) ||
        Array.from(probe.found.plugins?.values() || []).some((p) => p.id === id || p.id === cleanPluginId || p.rawId === id);
    } else if (type === "skill") {
      const slug = id.split("/").pop();
      stillThere =
        probe.found.skills?.has(id) ||
        probe.found.skills?.has(slug) ||
        Array.from(probe.found.skills?.values() || []).some((s) => s.id === id || s.id === slug);
    } else if (type === "mcp") {
      stillThere = probe.found.mcps?.has(id) ?? false;
    }
