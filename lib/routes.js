...
import { detectStack } from "./context.js";
...
// refresh time), so the sanitizer accepts legitimate CLI values â€” flags, http(s)
// URLs, quoted values containing ${VAR} placeholders and spaces, and plain
// paths â€” while rejecting shell execution metachars OUTSIDE of quoted segments.
 ...
 *   - skill  â†’ `cline skill remove <name> -y` (+ `-g` when global)
 *   - mcp    â†’ `cline [--config <cwd>/.cline] mcp uninstall <name> --json`
 *   - plugin â†’ `cline plugin uninstall <name> --json` (+ `--cwd <cwd>`)
 * @param {string} type
 * @param {string} id
...
export function createApiRouter({ root, dataDir, CATALOG_PATH, PREV_CATALOG_PATH, META_PATH, INSTALLED_PATH, WATCHLIST_PATH, CONTEXT_PATH, SETTINGS_PATH, controlToken = null, requireControlAuth = false }) {
  const router = Router();

  // Catalog is large (~196KB, 202+ entries) and re-read synchronously on every
  // GET /api/* that touches it. mtime-based cache (SSOT: catalog.json is only
  // replaced by refresh; sync-catalog CI bumps mtime) — measured ~24ms vs
  // ~986ms per 1000 reads (audit 2026-09-05 perf). Cleared on mutation.
  let _catalogCache = { mtime: 0, data: null };
  function loadCatalog() {
    const p = CATALOG_PATH;
    if (!existsSync(p)) return null;
    let mtime = 0;
    try { mtime = statSync(p).mtimeMs; } catch { mtime = Date.now(); }
    if (_catalogCache.mtime === mtime) return _catalogCache.data;
  ...
  // ---- Control-Token Auth (audit 2026-09-01 A1/A2) -------------------------
  // CLINEMARKET_CONTROL_TOKEN was only enforced as a startup guard, never on
  // routes. That left the entire control plane (install/uninstall/bulk/shutdown)
  // wide open on the LAN whenever it was exposed via ALLOW_REMOTE_HOST=1. This
  // middleware compares the token in constant time on every mutating route
  // (POST/PUT/DELETE/PATCH) when the server is in the exposed state
  // (requireControlAuth). It also closes the unguarded shutdown endpoint (A2):
  // by demanding the token when exposed, /api/shutdown can no longer be called
  // by an unauthenticated local caller.
  ...
  function controlTokenMiddleware(req, res, next) {
    if (!requireControlAuth) return next();
    const auth = req.get("authorization") || "";
    const provided = /^Bearer\s+/i.test(auth) ? auth.replace(/^Bearer\s+/i, "") : (req.get("x-control-token") || "");
    if (!controlToken || !safeTokenEqual(provided, controlToken)) {
      return res.status(401).json({ ok: false, error: "Unauthorized: valid control token required.", code: "UNAUTHORIZED" });
    }
    return next();
  }

  // Rate limit on mutating routes (audit A3): mitigates abuse of endpoints that
  // spawn the `cline` subprocess (install/bulk) when the server is exposed.
  const mutateLimiter = rateLimit({
    windowMs: 60_000,
    limit: 120,
    standardHeaders: true,
    legacyHeaders: false,
    message: { ok: false, error: "Too many mutating requests.", code: "RATE_LIMITED" },
  });

  // Apply to every mutation: auth first, then rate limit. Reads (GET) stay open.
  router.use((req, res, next) => {
    if (req.method === "GET" || req.method === "HEAD" || req.method === "OPTIONS") return next();
    controlTokenMiddleware(req, res, next);
  }, mutateLimiter);

  // ---- Context Analysis Engine ---------------------------------------------
  // Stack detection (repo/languages/frameworks/tags/hints/dependencies + git
  // head + package manager) lives in lib/context.js: unified engine shared
  // with scripts/detect-context.mjs (audit 2026-09-05, dedup of divergent
  // engines). This layer only adds catalog-driven recommendations.
  function analyzeWorkspaceContext(targetCwd) {
    ...
    if (!existsSync(cwd)) {
      return {
        ...
        commit: null,
        packageManager: null,
        localPrimitives: { skills: 0, plugins: 0, mcps: 0, total: 0 },
        languages: [],
        frameworks: [],
        tags: [],
        hints: [],
  ...
  // ---- Catalog Search (programmatic / scripting) -----------------------------
  // Full-text search across the catalog for developers querying from scripts
  // (pairs with `cline-marketplace status --json`). Query params:
  //   q        = search term (matches id, name, tagline, description, author, tags)
  //   type     = plugin|skill|mcp (optional filter)
  ...
  router.get("/search", (req, res) => {
    ...
    if (type === "plugin" || type === "skill" || type === "mcp") {
    ...
    if (q) {
      ...
      results = results.filter((e) => {
        const haystack = [
          e.name, e.tagline, e.description, e.author?.name, e.id, e.type, ...(e.tags || []),
        ].join(" ").toLowerCase();
    ...
    const paged = results.slice(offset, offset + limit).map((e) => ({
      key: e.key || `${e.type}:${e.id}`,
      type: e.type,
      id: e.id,
      name: e.name,
      tagline: e.tagline,
  ...
  router.get("/health", async (req, res) => {
    ...
    checks.push({
      name: "node",
      ok: true,
      detail: `${process.version} (${arch()}) Â· V8 ${process.versions.v8 || "active"}`,
  ...
  // Requirement 7: generous timeouts â€” skill installs clone GitHub repos.
