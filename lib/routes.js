...
export function createApiRouter({ root, dataDir, CATALOG_PATH, PREV_CATALOG_PATH, META_PATH, INSTALLED_PATH, WATCHLIST_PATH, CONTEXT_PATH, SETTINGS_PATH }) {
    ...
    if (existsSync(pkgPath)) {
      ...
      try {
        ...
        if (j.devDependencies?.typescript || j.dependencies?.typescript || existsSync(join(cwd, "tsconfig.json"))) {
          res.languages.push("typescript");
        }
        if (j.dependencies?.react || j.devDependencies?.react) res.frameworks.push("react");
        if (j.dependencies?.vue || j.devDependencies?.vue) res.frameworks.push("vue");
        if (j.dependencies?.next || j.devDependencies?.next) res.frameworks.push("nextjs");
        if (j.dependencies?.express || j.devDependencies?.express) res.frameworks.push("express");
        if (j.dependencies?.tailwindcss || j.devDependencies?.tailwindcss) res.frameworks.push("tailwind");
      } catch {}
    }

    if (existsSync(join(cwd, "pyproject.toml")) || existsSync(join(cwd, "requirements.txt")) || shallowFiles.some((f) => f.endsWith(".py"))) {
      res.languages.push("python");
    }
    if (existsSync(join(cwd, "go.mod")) || shallowFiles.some((f) => f.endsWith(".go"))) res.languages.push("go");
    if (existsSync(join(cwd, "Cargo.toml")) || shallowFiles.some((f) => f.endsWith(".rs"))) res.languages.push("rust");
    if (existsSync(join(cwd, "Dockerfile")) || existsSync(join(cwd, "docker-compose.yml"))) res.frameworks.push("docker");
    if (existsSync(join(cwd, ".git"))) res.frameworks.push("git");

    const cat = loadCatalog();
    if (cat?.entries) {
      const recSet = new Set();
      const allTokens = [...res.languages, ...res.frameworks].map((t) => t.toLowerCase());

      for (const entry of cat.entries) {
        const entryText = `${entry.id} ${entry.name} ${entry.description || ""} ${(entry.tags || []).join(" ")}`.toLowerCase();
        if (allTokens.some((tok) => entryText.includes(tok))) {
          recSet.add(entry.key || `${entry.type}:${entry.id}`);
        }
      }
      res.recommended = Array.from(recSet).slice(0, 15);
    }
  ...
  router.post("/settings", async (req, res) => {
    ...
    const updated = {
      ...
      ...(Array.isArray(b.recentWorkspaces) ? { recentWorkspaces: b.recentWorkspaces.slice(0, 20) } : {}),
    };
  ...
  router.post("/bulk", async (req, res) => {
    ...
    if (!["install", "uninstall", "watch", "unwatch"].includes(action)) {
      return res.status(400).json({ error: "Invalid action. Supported: install, uninstall, watch, unwatch." });
    }
  ...
  router.post("/refresh", async (req, res) => {
    try {
      ...
      const { stdout, stderr } = await execFileP(process.execPath, [refreshScript], {
        cwd: root,
        timeout: 60000,
        windowsHide: true,
      });
  ...
  router.post("/update/run", async (req, res) => {
    try {
      ...
      const { stdout: installOut } = await execFileP(npmExe, ["install", "--omit=dev"], { cwd: root, timeout: 60000 });
  ...
  router.post("/import", async (req, res) => {
    ...
    if (!Array.isArray(items)) {
      return res.status(400).json({ error: "Invalid import format. 'installed' array required." });
    }
    const state = loadInstalled();
    let added = 0;
    const now = new Date().toISOString();
