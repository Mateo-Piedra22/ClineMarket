...
export function resolveShimScript(shimPath) {
  ...
  try {
    ...
    const pushCandidate = (raw) => {
      ...
      const expanded = cleanRaw.replace(/%~dp0|%dp0%/gi, shimDir);
      if (expanded && /\.([cm]?js)$/i.test(expanded)) candidates.push(expanded);

      // Cross-platform normalization: handles Windows backslashes in .cmd wrappers on POSIX (Linux/macOS)
      const relWithoutDp0 = cleanRaw.replace(/%~dp0|%dp0%/gi, "").replace(/^[/\\]+/, "");
      if (relWithoutDp0 && /\.([cm]?js)$/i.test(relWithoutDp0)) {
        const normalizedRel = relWithoutDp0.replace(/\\/g, "/");
        candidates.push(join(shimDir, normalizedRel));
      }
    };
    ...
    for (const candidate of candidates) {
      ...
      try {
        ...
        const joinedNorm = resolve(shimDir, candidate.replace(/\\/g, "/"));
        if (existsSync(joinedNorm)) {
          resolved = joinedNorm;
          break;
        }
      } catch {}
